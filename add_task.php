...
if(isset($_SERVER["REQUEST_METHOD"]) && strip_tags($_SERVER["REQUEST_METHOD"]) == strip_tags("POST"))
{
	if(isset($_POST['add']))
	{
		if( isset($_POST['task']) && !empty($_POST['task']) ) // Be sure the task brought here is not an empty field
		{
			...
			$date = trim(strip_tags(htmlspecialchars($_POST['trip'])));
			// Save the task into the database
			mysqli_query($MYSQLi, "insert into `task` values(0, '".mysqli_real_escape_string($MYSQLi, $task)."', '".mysqli_real_escape_string($MYSQLi, 'Pending')."','".mysqli_real_escape_string($MYSQLi, $date)."')") or die(mysqli_errno());
